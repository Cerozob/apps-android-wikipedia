...
class NotificationActivity : BaseActivity() {
    ...
    private fun delimitedFilteredWikiList(): String {
        ...
        val filteredWikiList =
            NotificationsFilterActivity.allWikisList().filterNot { excludedWikiCodes.contains(it) }.map {
                val defaultLangCode =
                    WikipediaApp.instance.appLanguageState.getDefaultLanguageCode(it) ?: it
    ...
    private open inner class NotificationItemHolder constructor(val binding: ItemNotificationBinding) :
        ...
        private fun appLangCodesContains(langCode: String): Boolean {
            return WikipediaApp.instance.appLanguageState.appLanguageCodes.count {
                it == langCode || WikipediaApp.instance.appLanguageState.getDefaultLanguageCode(it) == langCode
            } > 0
        }
        ...
        override fun onClick(v: View) {
            if (MultiSelectActionModeCallback.isTagType(actionMode)) {
                ...
                n.contents?.links?.getPrimary()?.let { link ->
                    val url = link.url
                    if (url.isNotEmpty()) {
                        NotificationInteractionFunnel(WikipediaApp.instance, n).logAction(NotificationInteractionEvent.ACTION_PRIMARY, link)
